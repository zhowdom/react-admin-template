...
const Component = (props: any) => {
  ...
  const uploadedFileList = useRef<any[]>([]); // 多选暂存
  if(_ref) {
    _ref.current = {
      resetValue(info: any) {
        setOverFileList(info);
      }
    }
  }
  ...
  const handleUpload = async (data: any) => {
    ...
    if (multiple) {
      uploadedFileList.current = [...uploadedFileList.current, ...resList];
      allBack(uniqBy(uploadedFileList?.current || [], 'id'));
    } else {
      allBack(resList);
    }
  };
  ...
  return (
    <>
      <Upload
        ...
        listType={listType}
        {...uploadProps}
        fileList={hideFileList ? [] : overFileList}
        ...
        {props.disabled ? (
          ...
          <>
            {listType == 'picture-card' ? (
              ...
              <Button
                onClick={() => {
                  if (multiple) uploadedFileList.current = [];
                }}
