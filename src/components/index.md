...
export default () => {
  ...
  return (
    <NoticeIcon
      count={10}
      onItemClick={(item) => {
        pubMsg(`${item.title} 被点击了`, 'info');
      }}
      onClear={(title: string, key: string) => pubMsg('点击了清空更多', 'info')}
      loading={false}
      clearText="清空"
      viewMoreText="查看更多"
      onViewMore={() => pubMsg('点击了查看更多', 'info')}
      clearClose
    >
      <NoticeIcon.Tab
        tabKey="notification"
...
| 参数 | 说明 | 类型 | 默认值 |
| --- | --- | --- | --- |
| fileBack | 上传结束后 或 触发删除上传动作后的回调 | `()=>void` | - |
| businessType | 上传接口的 key,可在数据字典拿 | `string` | - |
| listType | 展示形式 | `picture/picture-card/text(默认)` | `true` |
| accept | 非必填 打开上传文件弹窗时，显示的文件类型，默认只显示指定的文件类型 可以和 acceptType 配套使用，也可以不用 | `boolean` | `true` |
| acceptType | 非必填 默认不限制上传文件类型 后辍名的数组 | `Array` | - |
| acceptMessage | 非必填 当上传类型错误时，指定自定义的提示信息，默认提示的信息 提示"上传格式不正确！ | `string` | - |
| maxSize | 非必填 上传大小限制 单位 M | `number` | - |
| maxCount | 非必填 上传数量限制 注意：当为 1 时，不提示数量超出而是直接替换原文件 | `number` | - |
| defaultFileList | 非必填 原数据回显 [] 直接用现在接口返回的数据 | `Array` | - |
| checkMain | 非必填 是否有设置主图的按钮操作 默认 false | `boolean` | - |
