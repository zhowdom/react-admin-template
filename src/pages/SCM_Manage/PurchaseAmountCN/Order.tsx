...
import { pubFilter, pubMsg, selectProps } from '@/utils/pubConfig';
import { pubBlobDownLoad, pubGetVendorList } from '@/utils/pubConfirm';
import { exportOrderCnList, getList } from '@/services/pages/SCM_Manage/purchaseAmountCN';
...
const PageOrder: React.FC<{
  ...
  const downLoad = async () => {
    if (!access.canSee('liyi99-report_purchase-amount-cn-export')) {
      pubMsg('您暂无"导出"权限, 可联系管理员开通哦~');
      return;
    }
    setDownLoading(true);
    const res: any = await exportOrderCnList(exportForm);
    setDownLoading(false);
    pubBlobDownLoad(res, '采购金额统计(采购下单维度)');
  };
  // 获取表格数据
  ...
  const columns: any[] = [
    ...
    },
    {
      title: (
        <>
          ...
          <Tooltip
            placement="top"
            title="筛选时间范围内，审核通过、待签约、已签约未入库三种状态的采购单的采购单总金额汇总（包括运费和采购单扣款）"
          >
            <QuestionCircleOutlined style={{ marginLeft: 4 }} />
          </Tooltip>
        </>
    ...
    },
    {
      title: (
        <>
          已部分收货（货值）
          <Tooltip placement="top" title="筛选时间范围内，已签约部分已入库采购单的采购单总金额汇总">
            <QuestionCircleOutlined style={{ marginLeft: 4 }} />
          </Tooltip>
        </>
      ),
    ...
    },
    {
      title: (
        <>
          已全部收货（货值）
          <Tooltip placement="top" title="筛选时间范围内，已签约全部已入库采购单的采购单总金额汇总">
            <QuestionCircleOutlined style={{ marginLeft: 4 }} />
          </Tooltip>
        </>
      ),
  ...
  return (
    <>
      <ProTable
        ...
        toolBarRender={() => [
          <Space key="space">
            {access.canSee('liyi99-report_purchase-amount-cn-export') ? (
              <Button
