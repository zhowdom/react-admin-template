...
const PageStock: React.FC<{
  ...
  const downLoad = async () => {
    ...
    setDownLoading(true);
    const res: any = await exportReceivingCnList(exportForm);
    setDownLoading(false);
    pubBlobDownLoad(res, '采购金额统计(收货入库维度)');
  };
  // 获取表格数据
  ...
  const columns: any[] = [
    ...
    },
    {
      title: (
        <>
          已收货数量
          <Tooltip placement="top" title="筛选条件内，该供应商所有采购单对应入库单的已入库商品数量">
            <QuestionCircleOutlined style={{ marginLeft: 4 }} />
          </Tooltip>
        </>
      ),
      ...
      render: (_: any, record: any) =>
        record?.warehousingNum ? (
          <ModalStock
            ...
            title={record.vendorName + ' - 已收货数量明细'}
            trigger={
              <a>
                <ProFormField
                  mode={'read'}
    ...
    },
    {
      title: (
        <>
          ...
          <Tooltip
            placement="top"
            title="筛选条件内，该供应商所有采购单对应入库单的已入库商品数量乘以对应商品采购单价"
          >
            <QuestionCircleOutlined style={{ marginLeft: 4 }} />
          </Tooltip>
        </>
