...
const Page: React.FC<{dicList: any, totalData?: any, fromTab?: boolean, time?: any}> = (props: any) => {
  const actionRef = useRef<ActionType>();
  const { dicList, totalData, fromTab } = props;
  ...
  console.log(totalData, 'history.location?.state');
  // table配置
  const columns: any[] = [
    {
      title: (
    ...
    },
    {
      ...
      width: 110,
      align: 'center',
      initialValue: fromTab ? '' : totalData.skuCode,
    ...
    },
    {
      ...
      initialValue: totalData && totalData.time ? {
        sType: totalData.time?.sType,
        dType: totalData.time?.dType,
        dateList: [moment(totalData.time?.dateList?.[0]), moment(totalData.time?.dateList?.[1])],
      }: {
        sType: 'sign',
        dType: 'date',
      ...
      search: {
        transform: (value: any) => {
          ...
          return{
            beginTime: value?.sType === 'create' && value?.dateList?.[0]
            ? `${moment(value?.dateList?.[0]).format('YYYY-MM-DD')} 00:00:00`
            : null,
            endTime:
              value?.sType === 'create' && value?.dateList?.[1]
                ? `${moment(value?.dateList?.[1]).format('YYYY-MM-DD')} 23:59:59`
                : null,
            beginSigningTime:
              value?.sType === 'sign' && value?.dateList?.[0]
                ? `${moment(value?.dateList?.[0]).format('YYYY-MM-DD')} 00:00:00`
                : null,
            endSigningTime:
              value?.sType === 'sign' && value?.dateList?.[1]
                ? `${moment(value?.dateList?.[1]).format('YYYY-MM-DD')} 23:59:59`
                : null,
          }
        },
