...
const Page: React.FC<{
  ...
  timeRangeSet: any;
  [k: string]: any;
}> = ({ history, common, onTabChange, goodsCodeSet, timeRangeSet, timeRange }) => {
  ...
  const downLoad = async () => {
    if (!access.canSee('liyi99-report_feedback_plat_export')) {
      pubMsg('您暂无"导出"权限, 可联系管理员开通哦~');
      return;
    }
    downloadingSet(true);
    const res: any = await api.platformPageListExport(exportForm);
    ...
    },
    {
      ...
      render: (_, record) => (
        <Chart
          params={{
            goodsId: record.goodsId,
            platformClassifyName: record.platformClassifyName,
            purchaseTimeStart: exportForm.purchaseTimeStart,
            purchaseTimeEnd: exportForm.purchaseTimeEnd,
          }}
          trigger={<a>{record.num}</a>}
          title={`客诉子分类: ${record.platformClassifyName} - 趋势图`}
          page={'KStongjiPlarm'}
        />
      ),
    },
    ...
    <>
      <ProTable
        ...
        request={async (params: any) => {
          ...
          const res = await api.platformPageList(formData);
        ...
        rowKey={(record: any) => record.goodsId + record.platformClassifyId}
        dateFormatter="string"
        toolBarRender={() => [
          <Space key={'tools'}>
            {access.canSee('liyi99-report_feedback_plat_export') ? (
              <Button
