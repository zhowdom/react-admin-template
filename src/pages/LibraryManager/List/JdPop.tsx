...
const Page = (props: any) => {
  ...
  const columns: ProColumns<any>[] = [
...

    {
      ...
      hideInSearch: true,
      render: (_: any, record: any) => (
        <ViewLife goods_sku_id={record.goods_sku_id} dicList={dicList} platform_code="JD_POP" />
      ),
      width: 145,
      hideInTable: !access.canSee('libraryManager_life_pop'),
    },
    {
      title: '平台',
    ...
    },
    {
      ...
      render: (_, record: any) => <InnerTable data={record.inventories} dicList={dicList} tabType={'jdPop'} />,
    },
    {
      title: (
        <>
  ...
  const synchronizationAction = async () => {
    pubModal('是否确定同步库存数据?')
      .then(async () => {
        ...
        const res: any = await api.syn({ platform_code: 'JD_POP' });
    ...
    <ProTable
      ...
      headerTitle={
        <Space key="space" wrap>
          <Access key="approval" accessible={access.canSee('libraryManager_sync_pop')}>
            <Button
          ...
          <Access key="exportInventory" accessible={access.canSee('libraryManager_export_pop')}>
            <Button
              type="primary"
              loading={downLoading}
              onClick={() => {
