...
const Page = (props: any) => {
      ...
      (entries) => {
        entries.forEach((item) => {
          ...
          if (item.isIntersecting) {
            ...
            console.log(item.target.rowSpan, 'item显示');
          } else {
            item.target.style.height = getStyleByAttr(item.target, 'height');
            
            item.target.querySelectorAll('td').forEach((v: any) => {
                console.log(v.rowSpan,'v.rowSpan')
                if(!v.rowSpan || v.rowSpan == 1) {
                    v.style.display = 'none';
                }
         
            });
            // console.log(item.target, 'item隐藏');
          }
        });
      },
      {
  ...
  const getAction = async (params: any) => {
    ...
    if (refreshColumns != dataFlat?.[0]?.details?.length) {
      refreshColumnsSet(dataFlat?.[0]?.details?.length);
    }
    // const dataFlatC = JSON.parse(JSON.stringify(dataFlat))
    // for(let i = 0;i<=900;i++) {
    //   dataFlat  = [...dataFlat,dataFlatC]
    // }
