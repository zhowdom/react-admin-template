...
const Account = (props: any) => {
  ...
  const getListAction = async (params: any): Promise<any> => {
    ...
    const res = await getAccountStatementOrderInPage(postData);
    if (res?.code != pubConfig.sCode) {
      pubMsg(res?.message);
    }
    setSelectRows([]);
  ...
  const columns: ProColumns<TableListItem>[] = [
    ...
    },
    {
      ...
      render: (_, row) => {
        const renderList = [];
        renderList.push(
          <Access key="detail" accessible={access.canSee('accountStatementOrder_in_detail')}>
            <a
              onClick={() => {
                history.push(`/reconciliation-manage/purchaseIn-detail?id=${row.id}`);
              }}
            >
              详情
            </a>
          ...
          //   key="paymentButton"
          //   accessible={
          //     access.canSee('accountStatementOrder_in_payment') && row.approval_status == '8'
          //   }
          // >
          //   <a
          //     onClick={() => {
          ...
          //   </a>
          // </Access>,
          <Access key="auditList" accessible={access.canSee('accountStatementOrder_in_history')}>
            <a
              onClick={() => {
          ...
          <ProTable<TableListItem>
            ...
            headerTitle={
              <Space key="space" wrap>
                <Access
                  key="access"
                  accessible={access.canSee('accountStatementOrder_in_paymentList')}
                >
                  <Button
            ...
            toolBarRender={() => [
              <Space key="space">
                <Access
                  key="printButton"
                  accessible={access.canSee('accountStatementOrder_in_print_list')}
                >
                  <Button
                    key="print"
                    type="primary"
                ...
                <Access
                  key="cancelButton"
                  accessible={access.canSee('accountStatementOrder_in_export')}
                >
                  <Button
                    key="cancel"
                    type="primary"
                ...
                <Access key="addButton" accessible={access.canSee('accountStatementOrder_in_add')}>
                  <Button
                    key="add"
                    type="primary"
                    onClick={() => {
