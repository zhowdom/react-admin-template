...
const All: React.FC<{
  ...
  const exportExcel = async () => {
    exportingSet(true);
    const res: any = await shippingMethodApprovalExport(Object.assign({ type: 'shipping_method' }, queryParams));
    exportingSet(false);
  ...
  const getListAction = async (params: any): Promise<any> => {
    ...
    const res = await shippingMethodApprovalPage(Object.assign({ type: 'shipping_method' }, postData));
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <Space direction={'vertical'}>
          {access.canSee('safe_stock_shipping_trafic_approve') || access.canSee('safe_stock_shipping_trafic_reject') ? (
            <>
              <Popconfirm
                title={'确定同意?'}
                onConfirm={async () => {
          ...
          {currentUser?.name == record.create_user_name &&
            access.canSee('safe_stock_shipping_trafic_cancel') ? (
            <a
              onClick={() => {
  ...
  return (
    <>
      <ProTable
        ...
        headerTitle={
          <Space>
            {access.canSee('safe_stock_shipping_trafic_batchapprove') || access.canSee('safe_stock_shipping_trafic_batchreject') ? (
              <>
            ...
            <Access
              key="safe_up_safeStock_cancel_list"
              accessible={access.canSee('safe_up_safeStock_cancel_list')}
            >
              <Button
                type={'primary'}
            ...
            <Access key="export" accessible={access.canSee('safe_up_export_approval')}>
