...
const Page: React.FC<{ common: any }> = ({common}) => {
  ...
  const onSubmit = async (approve_status: 'NEW' | 'WAIT_APPROVAL') => {
    ...
    return editorFormRef.current?.validateFields().then(async () => {
      const postData = {
        id: id || '',
        "order_type": "USE_PARTS_STOCK_OUT", // 字典: SCM_OWN_STOCK_ORDER_TYPE
      ...
      if (res?.code == pubConfig.sCode) {
        ...
        history.replace('/warehouse/output/po')
  ...
  return <>
    {
      access.canSee('warehouse-output-po-detail')
          ...
          submitter={{
            render: () => {
              return !id || edit || approval ? (
                <FooterToolbar>
                  <Button icon={<ArrowLeftOutlined/>} onClick={() => history.replace('/warehouse/output/po')}>
                    返回
                  </Button>
                  {
                    approval ? <>
        ...
        >
          ...
          <Card title={`出库商品明细 - 包材配件领用`}>
            <EditableProTable
              headerTitle={
                edit || !id ?
                  <SelectGood
                    title={'添加商品(可用的包材配件)'}
                    trigger={<Button type={'primary'}>添加商品(可用的包材配件)</Button>}
                    requestApi={'/sc-scm/ownStockManagement/ownStockPage'}
                    requestParams={{stock_type: 'PARTS', queryAvailableGoodQty: true}}
                    extraColumns={[
                      {title: '包材配件可用数量', dataIndex: 'availableGoodQty', align: 'center', width: 90, hideInSearch: true,},
                    ]}
                    value={goodsList}
                    onChange={(val: any[]) => {
                      const formatVal = val.map((data: any) => {
                        return {...data, availableQty: data.availableGoodQty, qty: 0, reason: ''}
                      })
              ...
              columns={[
                ...
                {title: '包材配件可用数量', dataIndex: 'availableQty', editable: false,},
                {
                  title: `领用出库数量`,
                  dataIndex: 'qty',
                  width: 110,
