...
import { uploadSignFileR } from '@/services/pages/purchaseOrder';
import { pubConfig, pubMsg } from '@/utils/pubConfig';
import { uploadContractR } from '@/services/pages/updateOrder';

const Dialog = (props: any) => {
  const formRef = useRef<ProFormInstance>();
  const [loading, setLoading] = useState(false);
  const title =
    props?.data?.approval_status == '5' ? '上传线下签约合同-R' : '重新上传线下签约合同-R';

  // 提交
  const saveSubmit = async (data: any): Promise<any> => {
    data.id = props?.data.id;
    const res = props.change
      ? await uploadContractR({ id: data.id, contract: data?.sys_files })
      : await uploadSignFileR(data);
