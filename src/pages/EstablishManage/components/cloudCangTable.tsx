...
const CloudCangTable = (props: any) => {
  ...
  const fileColumns: ProColumns<any>[] = [
    {
      ...
      render: (_: any,record: any) => {
        return disabled ? record?.sku_name??'-' : `${productName}${record?.sku_form_spec?.sku_form??''}${record?.sku_form_spec?.sku_spec??'-'}`
      },
      hideInTable:
  ...
  useEffect(() => {
    const data = props.formRef.current.getFieldValue('projectsCloudCangData');
              ...
              editable={{
                ...
                onValuesChange: (record, recordList) => {
                  props?.formRef?.current?.setFieldsValue({
                    projectsCloudCangData: recordList,
                    projectsGoodsSkus: props?.formRef?.current?.getFieldValue('projectsGoodsSkus').map((v: any) => ({
                      ...v,
                      cloud_warehouse_id:
